...
class D1UsageRepo implements UsageRepo {
  ...
  async query(opts: { keyId?: string; start: string; end: string }): Promise<UsageRecord[]> {
    const sql = opts.keyId
      ? "SELECT key_id, model, hour, requests, input_tokens, output_tokens, COALESCE(cache_read_tokens, 0) as cache_read_tokens, COALESCE(cache_creation_tokens, 0) as cache_creation_tokens FROM usage WHERE key_id = ? AND hour >= ? AND hour < ? ORDER BY hour"
      : "SELECT key_id, model, hour, requests, input_tokens, output_tokens, COALESCE(cache_read_tokens, 0) as cache_read_tokens, COALESCE(cache_creation_tokens, 0) as cache_creation_tokens FROM usage WHERE hour >= ? AND hour < ? ORDER BY hour";
  ...
  async listAll(): Promise<UsageRecord[]> {
    const { results } = await this.db
      .prepare("SELECT key_id, model, hour, requests, input_tokens, output_tokens, COALESCE(cache_read_tokens, 0) as cache_read_tokens, COALESCE(cache_creation_tokens, 0) as cache_creation_tokens FROM usage ORDER BY hour")
      .all<{ key_id: string; model: string; hour: string; requests: number; input_tokens: number; output_tokens: number; cache_read_tokens: number; cache_creation_tokens: number }>();
    return results.map((r) => ({
      keyId: r.key_id,
      model: r.model,
